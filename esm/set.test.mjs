...
describe('set', function() {
  it('sets value as expected when path is array', function() {
    const object = {
  ...
  it('sets value as expected when path is string', function() {
    const object = {
      a: { a: { a: 'AAA' }, b: [{ a: 'AB1A', b: 'AB1B' }, { a: 'AB2A' }] },
      b: { a: { a: 'BAA' } },
    };
  ...
  it('sets value as expected when path is empty array', function() {
    const object = { a: { a: { a: 'AAA' } } };
    const actual = set(object, [], 'Modified');
    expect(actual).not.toBe(object);
    expect(actual).toEqual('Modified');
  });
  it('sets value as expected when path is empty string', function() {
    const object = { a: { a: { a: 'AAA' } } };
    const actual = set(object, '', 'Modified');
    expect(actual).not.toBe(object);
    expect(actual).toEqual('Modified');
  ...
  it('throws expected error when path is not given', function() {
    expect(() => set({})).toThrow(new TypeError('Invalid arguments'));
  });
